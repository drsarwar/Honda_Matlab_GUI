...
ylim=[0.04 0.1]; %for pressure
%ylim=[-0.05 0.05]; %for shear
...
matmap = 1+[blank p2 p1 blank;p3 left prox p8;p4 bottom right p7;blank p5 p6 blank]; %pizza sensors
matmap1=matmap(:);
...
set(ha(6),'YLim', [-0.5 .7]); % different ylim for normal strain
set(ha(7),'YLim', [8.5 13]); %different ylim for proximity
set(ha(10),'YLim', [-0.04 .04]);
set(ha(11),'YLim', [-0.04 .04]);
drawnow;
    ...
    if line(1)=='('
        ...
        elseif (count > 16)
            ...
            elseif(loc==right)
                loc = bottom;
                x_C2_prime = val;
                val = (x_C2*x_C1_prime-x_C1*x_C2_prime)/(x_C1_prime + x_C2_prime);
            %top
            elseif(loc==top)
                y_C1_prime = val;
                %val = (x_C1_prime - x_C1 + x_C2_prime - x_C2 + y_C1_prime - y_C1 + y_C2_prime - y_C2)/(x_C1_prime + x_C2_prime + y_C1_prime + y_C2_prime); % decoupled pressure
            ...
            elseif(loc==bottom)
                loc = right;
                y_C2_prime = val;
                val = (y_C2*y_C1_prime-y_C1*y_C2_prime)/(y_C1_prime + y_C2_prime);
            end
        end
